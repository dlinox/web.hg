<template>
  ...
  >
    <div class="relative container w-full py-8 lg:py-24 text-center z-40">
      <h1 class="text-5xl mb-3 text-primary">Noticias</h1>
  ...
  <section>
    <div class="relative items-center w-full py-12 lg:py-32">
      <div class="container grid grid-cols-1 gap-8 md:grid-cols-2">
        ...
        <div v-for="(item, index)  in items" :key="index" class="flex font-sans bg-white shadow-md rounded-lg">
...
const init = async () => {
  ...
  let servicesRes = await fetch(
    "https://goyeneche.linox.net.pe/api/news"
  );
